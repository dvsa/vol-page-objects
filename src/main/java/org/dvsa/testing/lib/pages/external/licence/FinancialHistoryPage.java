...
public class FinancialHistoryPage extends BasePage {
    ...
    public static void declaredBankrupctyInThePast(boolean hasBeenBankruptBefore) throws MissingDriverException {
        click(String.format(DECLARED_BANKRUPT_TEMPLATE, radioPosition(hasBeenBankruptBefore)));
    }

    public static void InvolvedWithBusinessGoneIntoLiquidation(boolean involved) throws MissingDriverException {
        click(String.format(INVOLVED_WITH_A_BUSINESS_GONE_INTO_LIQUIDATION_TEMPLATE, radioPosition(involved)));
    }

    public static void involvedWithBusinessInReceivership(boolean involved) throws MissingDriverException {
        click(String.format(INVOLVED_WITH_BUSINESS_IN_RECEIVERSHIP, radioPosition(involved)));
    }

    public static void involvedWithCompanyGoneIntoAdministrationOrVoluntaryAgreement(boolean involved) throws MissingDriverException {
        click(String.format(INVOLVED_WITH_COMPANY_GONE_INTO_ADMINISTRATION_OR_VOLUNTARY_AGREEMENT, radioPosition(involved)));
    }

    public static void disqualifiedThroughCompanyDirectorsDisqualifiedAct1986(boolean disqualified) throws MissingDriverException {
