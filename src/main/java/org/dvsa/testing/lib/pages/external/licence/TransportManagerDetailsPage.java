...
public class TransportManagerDetailsPage extends BasePage {
    ...
    // Behaviour
    public static void dateOfBirth(@NotNull Calendar field, @NotNull String date) throws UninitialisedDriverException {
        enterField(String.format(DATE_OF_BIRTH_TEMPLATE, field.ordinal() + 1), date);
    }

    public static void emailAddress(@NotNull String emailAddress) throws UninitialisedDriverException {
        enterField(EMAIL_ADDRESS, emailAddress);
    }

    public static void placeOfBirth(@NotNull String place) throws UninitialisedDriverException {
    ...
    private static void findHomeAddressButton() throws UninitialisedDriverException {
        click(FIND_HOME_ADDRESS_BUTTON);
    }

    public static void homeAddress(@NotNull String optionValue) throws UninitialisedDriverException {
    ...
    private static void findWorkAddressButton() throws UninitialisedDriverException {
        click(FIND_WORK_ADDRESS_BUTTON);
    }

    public static void workAddress(@NotNull String optionValue) throws UninitialisedDriverException {
        list(FOUND_WORK_ADDRESSES_LIST, optionValue);
    }

    public static void typeOfManager(@NotNull TypeOfManager typeOfManager) throws UninitialisedDriverException {
        click(String.format(TYPE_OF_MANAGER, typeOfManager.ordinal() + 1));
    }

    public static void isOwnerOrDirector(boolean isOwnerOrDirector) throws UninitialisedDriverException {
        int elementPosition = isOwnerOrDirector ? 1 : 2;
        click(String.format(OWNER_OR_DIRECTOR, elementPosition));
    }

    public static void transportManagerTimeAllocation(@NotNull DayOfWeek dayOfWeek, float hours) throws UninitialisedDriverException {
        enterField(String.format(TRANSPORT_MANAGER_TIME_ALLOCATION, dayOfWeek.getValue()), String.valueOf(hours));
    }

    public static void reason(@NotNull String reason) throws UninitialisedDriverException {
    ...
    public static void saveAndReturnToTransportManagers() throws UninitialisedDriverException {
